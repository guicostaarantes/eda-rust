...
mod utils {
    pub(crate) fn auth_url(path: &str) -> String {
        let base_url =
            std::env::var("AUTH_BASE_URL").unwrap_or("http://localhost:7001".to_string());
        format!("{}{}", base_url, path)
    }

    pub(crate) fn astronauts_url(path: &str) -> String {
        let base_url =
            std::env::var("ASTRONAUTS_BASE_URL").unwrap_or("http://localhost:7002".to_string());
        format!("{}{}", base_url, path)
    }

    pub(crate) fn missions_url(path: &str) -> String {
        let base_url =
            std::env::var("MISSIONS_BASE_URL").unwrap_or("http://localhost:7003".to_string());
        format!("{}{}", base_url, path)
    }
